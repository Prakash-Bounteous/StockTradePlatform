...
public class WatchlistService {
    ...
    public String addToWatchlist(String username, String symbol) {
        ...
        watchlistRepository.findByUserIdAndStockId(user.getId(), stock.getId())
                .ifPresent(w -> { throw new RuntimeException("Already in watchlist"); });
        watchlistRepository.save(Watchlist.builder().user(user).stock(stock).build());
    ...
    public String removeFromWatchlist(String username, String symbol) {
        User user = userRepository.findByUsername(username).orElseThrow();
        Stock stock = stockRepository.findBySymbol(symbol).orElseThrow();
        Watchlist watchlist = watchlistRepository.findByUserIdAndStockId(user.getId(), stock.getId()).orElseThrow();
