...
public class AnalyticsService {
    ...
    public Map<String, BigDecimal> calculatePnL(String username) {
        ...
        for (Portfolio portfolio : portfolios) {
            BigDecimal currentPrice = portfolio.getStock().getPrice();
            BigDecimal averagePrice = portfolio.getAveragePrice() == null ? currentPrice : portfolio.getAveragePrice();
            BigDecimal profitLoss = currentPrice.subtract(averagePrice)
                    .multiply(BigDecimal.valueOf(portfolio.getQuantity()));
            pnl.put(portfolio.getStock().getSymbol(), profitLoss);
